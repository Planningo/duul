      ...
      try {
        ...
        if (!response.ok) {
          ...
          if ((status === 429 || status >= 500) && attempt < MAX_RETRIES - 1) {
            ...
            console.error(`[peer-reviewer] Anthropic retry ${attempt + 1}/${MAX_RETRIES} after ${delay}ms (status ${status})`);
        ...
        return { parsed, reviewId: body.id };
      } catch (error: unknown) {
        clearTimeout(timeout);
        if (attempt < MAX_RETRIES - 1 && error instanceof Error && error.name === 'AbortError') {
          const delay = 1000 * Math.pow(2, attempt);
          console.error(`[peer-reviewer] Anthropic retry ${attempt + 1}/${MAX_RETRIES} after ${delay}ms (timeout)`);
          await new Promise((resolve) => setTimeout(resolve, delay));
          continue;
        }
        throw error;
