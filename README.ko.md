...
호출 에이전트는 각 단계에서 `APPROVE` 판정을 받을 때까지 리뷰어와 반복하고, 이후 다음 단계로 진행합니다. 이를 통해 한 LLM이 다른 LLM의 작업을 검증하는 크로스 모델 리뷰 워크플로우를 만듭니다. 각 단계에는 무한 루프 방지를 위한 설정 가능한 반복 제한(plan/code: 7, partition: 5)이 있습니다.
...
대화 중에 **"DUUL"** (또는 **"두울"**)을 언급하면 활성화됩니다. 서버가 워크플로우 지시사항을 내장하고 있어 MCP 클라이언트가 자동으로 인식합니다.
...
| `api_key` | `string` | 요청별 API 키 (환경 변수 기반 키 해석을 오버라이드) |
| `temperature` | `number` (0-2) | 샘플링 온도 |
| `top_p` | `number` (0-1) | 핵 샘플링 파라미터 |

**우선순위:** 요청별 `reviewer_config` (`api_key` 포함) > 환경 변수 (`REVIEW_PROVIDER`, `REVIEW_MODEL`, `REVIEW_API_KEY`) > 기본값 (`openai`, `gpt-5.4`).

## 반복 제한

각 단계별 기본 반복 제한: **plan: 7, code: 7, partition: 5**. 제한을 초과하면 서버가 `requires_human_review: true`와 `iteration_limit_reached: true`를 반환하여 호출자가 사람에게 에스컬레이션할 수 있습니다.

**의미론:** `iteration_count > limit`일 때 호출이 차단됩니다 (이전 호출에서 이미 제한에 도달). `iteration_count === limit`일 때는 리뷰가 실행되고 (마지막 허용 반복) `iteration_limit_reached`는 `false`입니다. *다음* 호출이 차단됩니다.

**설정:**
- **요청별:** 도구 입력에 `max_review_iterations`를 전달하여 해당 호출만 오버라이드 (범위: 1–20).
- **환경 변수:** `MAX_PLAN_REVIEW_ITERATIONS`, `MAX_CODE_REVIEW_ITERATIONS`, `MAX_PARTITION_ITERATIONS`.
- **우선순위:** 요청별 오버라이드 > 환경 변수 > 기본값.
...
| `MAX_PARTITION_ITERATIONS` | 아니오 | `5` | 사람 개입 전 최대 실행 분할 반복 횟수 |
